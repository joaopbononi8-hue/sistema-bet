...
def get_matches():
    ...
    try:
        ...
        for jogo in jogos:
            ...
            # Calcular médias do time da casa
            cursor.execute('''
            ...
            medias_casa = cursor.fetchone()
            
            # Calcular médias do time de fora
            cursor.execute('''
                SELECT AVG(escanteios) as media_escanteios,
                       AVG(cartoes_amarelos) as media_amarelos
                FROM estatisticas
            ...
            medias_fora = cursor.fetchone()
            
            # Média combinada dos dois times
            if medias_casa and medias_fora:
                media_escanteios = (medias_casa['media_escanteios'] + medias_fora['media_escanteios']) / 2
                media_amarelos = (medias_casa['media_amarelos'] + medias_fora['media_amarelos']) / 2
                
                if media_escanteios and media_escanteios > 0:
                    matches_data.append({
